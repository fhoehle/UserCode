...
#include "TopQuarkAnalysis/TopJetCombination/plugins/TtSemiLepHypOwnPseudoMatchBJetCalcNu.h" // OwnPseudoMatchBJetCalcNu
#include "TopQuarkAnalysis/TopTools/interface/MyMEzCalculator.h"
#include "FWCore/MessageLogger/interface/MessageLogger.h"
#include "Math/VectorUtil.h"
TtSemiLepHypOwnPseudoMatchBJetCalcNu::TtSemiLepHypOwnPseudoMatchBJetCalcNu(const edm::ParameterSet& cfg):
...
TtSemiLepHypOwnPseudoMatchBJetCalcNu::~TtSemiLepHypOwnPseudoMatchBJetCalcNu() { }

void
TtSemiLepHypOwnPseudoMatchBJetCalcNu::buildHypo(edm::Event& evt,
...
// 	std::cout<<"two neutrino sols set "<<nuzSols.size()<<std::endl;
	const math::XYZTLorentzVector nuSol = ( fabs( topMass_ - ( (*leps)[0].p4() + nuSol1 + (*jets)[blepMatch_Idx].p4() ).M()) < fabs( topMass_ - ( (*leps)[0].p4() + nuSol2 + (*jets)[blepMatch_Idx].p4() ).M() ) ) ? nuSol1 : nuSol2;
