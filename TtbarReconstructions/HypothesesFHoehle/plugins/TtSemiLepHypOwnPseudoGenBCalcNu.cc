...
TtSemiLepHypOwnPseudoGenBCalcNu::TtSemiLepHypOwnPseudoGenBCalcNu(const edm::ParameterSet& cfg):
...
TtSemiLepHypOwnPseudoGenBCalcNu::~TtSemiLepHypOwnPseudoGenBCalcNu() { }

void
TtSemiLepHypOwnPseudoGenBCalcNu::buildHypo(edm::Event& evt,
...
// 	std::cout<<"two neutrino sols set "<<nuzSols.size()<<std::endl;
	const math::XYZTLorentzVector nuSol = ( fabs( topMass_ - ( (*leps)[0].p4() + nuSol1 + blep_Gen->p4() ).M()) < fabs( topMass_ - ( (*leps)[0].p4() + nuSol2 + blep_Gen->p4() ).M() ) ) ? nuSol1 : nuSol2;
