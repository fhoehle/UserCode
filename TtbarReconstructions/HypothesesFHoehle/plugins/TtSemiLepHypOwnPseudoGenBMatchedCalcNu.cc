...
TtSemiLepHypOwnPseudoGenBMatchedCalcNu::TtSemiLepHypOwnPseudoGenBMatchedCalcNu(const edm::ParameterSet& cfg):
...
TtSemiLepHypOwnPseudoGenBMatchedCalcNu::~TtSemiLepHypOwnPseudoGenBMatchedCalcNu() { }

void
TtSemiLepHypOwnPseudoGenBMatchedCalcNu::buildHypo(edm::Event& evt,
				     ...
				     std::vector<int>& match, const unsigned int iComb)
{
  ...
  evt.getByLabel("genEvt", genEvt);
	if(genEvt->isSemiLeptonic()  ) edm::LogWarning("genBInfo")<<" genEvt->leptonicDecayB()->pdgId() "<<genEvt->leptonicDecayB()->pdgId()<<" px "<<(genEvt->leptonicDecayB() ? genEvt->leptonicDecayB()->px() : -1.234567);
  if(leps->empty() || mets->empty() ||jets->empty() || jets->size() < 1 || !genEvt->isSemiLeptonic()|| genEvt->leptonicDecayB() == 0 ){
	...
	if(!genEvt->isSemiLeptonic() || genEvt->leptonicDecayB() == 0 ) edm::LogWarning("genInfo")<<"genEvt->leptonicDecayB() "<<genEvt->leptonicDecayB()<<" genEvt->isSemiLeptonic() "<<genEvt->isSemiLeptonic();
    return;
  }
...
// 	std::cout<<"two neutrino sols set "<<nuzSols.size()<<std::endl;
	const reco::GenParticle * nu = genEvt->singleNeutrino();
	const math::XYZTLorentzVector nuSol = ROOT::Math::VectorUtil::DeltaR( nu->p4(), nuSol1 ) < ROOT::Math::VectorUtil::DeltaR( nu->p4(), nuSol2 ) ? nuSol1 : nuSol2;
...
//    	match[TtSemiLepEvtPartons::LepB] = blepMatch_Idx;
	...
	edm::Ptr<pat::Jet> ptr_blep = edm::Ptr<pat::Jet>(jets, 0);
	const math::XYZTLorentzVector p4_blep = blep_Gen->p4();
	leptonicB_ = new reco::ShallowClonePtrCandidate( ptr_blep, ptr_blep->charge(), p4_blep, ptr_blep->vertex() );
  }
